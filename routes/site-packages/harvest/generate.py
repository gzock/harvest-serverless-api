...
from harvest.excel_doc_generator import ExcelDocGenerator
...
class Generate():
  ...
  def gen_zip(self, project_id, workspace="/tmp", result_name="photos_result.zip", need_download_link=True):
  ...
  def gen_excel_doc(self, project_id, need_custom_template=False, workspace="/tmp", template="template_1.xlsx", result="photos_result.xlsx", need_download_link=True):
