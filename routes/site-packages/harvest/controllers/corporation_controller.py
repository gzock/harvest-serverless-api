...
class CorporationController():
  ...
  def __init__(self, host, port):
    ...
    self.project_user = ProjectUserController(host, port)
  ...
  def list_corporations(self):
    corporations = self.cognito.list_groups()
    return [corp["GroupName"] for corp in corporations["Groups"]]
  ...
  def create_user(self, corporation_id, username, kana_username, email, password):
    ...
    if user["Enabled"]:
      ...
      self.logger.info("update billing record... users count and price is to be increment.")
      try:
        self.billing.update(event="create_user", user_id=corporation_id)
      except:
        self.logger.info("billing update process has been failed. new created user is to be delete.")
  ...
  def delete_user(self, user_id):
    ...
    self.project_user.set_user_id(user_id)
    self.logger.info("target delete user_id: " + user_id)

    # TODO: if user has project too many, project delete action is too long time. 
    #       in that case, over the timeout for api gateway or lambda.
    #       don't delete the user project. disable user first. 
    #
    # delete projects(owner)
    #for project in self.project.list_projects():
    #  self.logger.info("target delete project_id: " + project["project_id"])
    #  if project["role"] == "owner":
    #    self.logger.info("that project role is owner. delete to all attributes.")
    #    ret = self.project.set_project_id(project["project_id"])
    #    ret = self.project.delete()
    #    self.logger.info("result: " + str(ret))
    #  else:
    #  # delete roles(without owner)
    #    self.logger.info("that project role is other than owner. remove to project.")
    #    self.project_user.set_project_id(project["project_id"])
    #    ret = self.project_user.delete()
    #    self.logger.info("result: " + str(ret))
    #  delete cognito user
    #ret = self.cognito.delete_user(user_id)
    #  disable cognito user
    ret = self.cognito.disable_user(user_id)

    if ret["ResponseMetadata"]["HTTPStatusCode"] == 200:
      self.logger.info("disable user processing successfully: " + str(ret))
    else:
      self.logger.error("disable process failed... result" + str(ret))
      # TODO: 専用例外を作るべき
      raise Exception
