...
class PhotoController(DynamoDbDriver):
  ...
  def create(self, target_id, type, data):
    ...
    ret = self.__s3.put_obj(new_id, data, "project_id={}&target_id={}&type={}".format(self.project_id, target_id, type))
    # 何が返ってくればOK？例外処理が必要
    print(ret)
  ...
  def delete(self, target_id, photo_id):
    ...
    for key in ["before", "after"]:
      ...
      # TODO: 例外を返す
      print("error: photo_id not found")
      raise Exception
    ...
    if len(target["photos"][type]) == 1:
      print("error: last one")
      raise Exception

    # 採用写真なら消さない
    # TODO: 例外を返す
    if target["photos"]["adopt"][type] == photo_id:
      print("error: adoption photo")
      raise Exception
