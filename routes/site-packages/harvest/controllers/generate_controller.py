...
class GenerateController():
  ...
  def __get_photo_id_list(self, target, photo_type, all_photos=False):
    ...
    if all_photos:
      ...
      ret = [ target["photo_id"] if isinstance(target["photos"]["adopt"][photo_type], str) ]
    self.logger.info("generate photo id list: %s" % str(ret))
    return ret

  def __get_photo_list(self, project_id, by_name=True, needs_sort=True, reverse=False, needs_include_hierarchy=False, all_photos=False):
    ...
    for target in targets:
      ...
      for photo_type in ["before", "after"]:
        for photo_id in self.__get_photo_id_list(target, photo_type, all_photos=all_photos):
  ...
  def gen_zip(self, 
    ...
    self.logger.info("function argument 'all_photos': {}".format(all_photos))
    ...
    with ZipFile(local_result_path, 'w') as result_zip:
      for photo in self.__get_photo_list(project_id, by_name=by_name, needs_sort=False, needs_include_hierarchy=needs_include_hierarchy, all_photos=all_photos):
