...
class GenerateController():
  ...
  def list(self, project_id=None):
    ...
    gen_file_names = [ obj["Key"].split("/")[-1] for obj in objs if obj["Key"] != project_id ]
    print(gen_file_names)
    for name in gen_file_names:
      tags = self.__s3_gen.get_obj_tags(project_id + "/" + name)
      ret.append(tags)
    return ret
  ...
  def gen_zip(self, 
    ...
    if title:
      result_filename = title + ".zip"
    else:
      result_filename = str(uuid4()) + ".zip"
  ...
  def gen_excel_doc(self, 
    ...
    if title:
      result_filename = title + ".xlsx"
    else:
      result_filename = str(uuid4()) + ".xlsx"
    local_result_path = os.path.join(local_workspace_path, result_filename)
  ...
  def gen_download_url(self, generated_filename, project_id=None):
    if not project_id:
      project_id = self.project_id
    url = self.__s3_gen.create_presigned_url(project_id + "/" + generated_filename)
    self.logger.info("generated download url: {}".format(url))
