...
class GenerateController():
  ...
  def __get_photo_id_list(self, target, photo_type, all_photos=False):
    ...
    if all_photos:
      ...
      if isinstance(target["photos"]["adopt"][photo_type], str):
        ret.append(target["photo_id"])
    self.logger.info("generate photo id list: %s" % str(ret))
    return ret

  def __get_photo_list(self, project_id, by_name=True, needs_sort=True, reverse=False, needs_include_hierarchy=False, needs_all_photos=False):
  ...
  def gen_zip(self, 
    ...
    with ZipFile(local_result_path, 'w') as result_zip:
      for photo in self.__get_photo_list(project_id, by_name=by_name, needs_sort=False, needs_include_hierarchy=needs_include_hierarchy, needs_all_photos=needs_all_photos):
  ...
  def gen_excel_doc(self, project_id, needs_sort=True, needs_include_hierarchy=False, workspace="/tmp", template="basic_1.xlsx", result_filename="photos_result.xlsx", needs_download_url=True):
    ...
    photo_list = self.__get_photo_list(project_id, needs_sort=needs_sort, needs_include_hierarchy=needs_include_hierarchy)
