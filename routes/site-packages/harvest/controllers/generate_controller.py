    ...
    with ZipFile(local_result_path, 'w') as result_zip:
      for photo in self.__get_photo_list(project_id, by_name=by_name, needs_sort=False):
        ...
        try:
          ...
          if char_enc == "shift_jis":
            result_zip.write(filename=filename, arcname="{}_{}.jpg".format(
                photo["name"], photo["type"]).encode('shift_jis', errors='ignore').decode('shift_jis', errors='ignore')
            )
          else:
