...
class GenerateController():
  ...
  def __get_photo_list(self, project_id, by_name=True, needs_sort=True, reverse=False, needs_include_hierarchy=False):
  ...
  def gen_zip(self, project_id, by_name=True, needs_include_hierarchy=False, needs_make_dir=True, workspace="/tmp", result_filename="photos_result.zip", needs_download_url=True, char_enc="utf_8"):
    ...
    with ZipFile(local_result_path, 'w') as result_zip:
      for photo in self.__get_photo_list(project_id, by_name=by_name, needs_sort=False, needs_include_hierarchy=needs_include_hierarchy):
