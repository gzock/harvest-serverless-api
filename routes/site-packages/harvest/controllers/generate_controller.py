...
class GenerateController():
  ...
  def list(self, project_id=None):
    ...
    for obj in generated_files:
      ...
      tags["name"] = b64decode(tags["name"]).decode("utf-8")
      ret.append(tags)
    return ret
  ...
  def gen_zip(self, 
    ...
    self.logger.info("workspace disk free space: {} MB".format( (shutil.disk_usage(workspace).free / 1024 /1024) ))
    ...
    tags = {
      ...
      "name": b64encode(generated_filename.encode("utf-8")).decode("utf-8"),
      "type": "zip"
    }
  ...
  def gen_excel_doc(self, 
    ...
    if ".xlsx" not in local_template_path:
      local_template_path += ".xlsx"
    ...
    self.logger.info("function argument 'needs_include_hierarchy': {}".format(needs_include_hierarchy))
    self.logger.info("function argument 'workspace': {}".format(workspace))
    self.logger.info("function argument 'template_id': {}".format(template_id))
    self.logger.info("function argument 'title': {}".format(title))
    self.logger.info("function argument 'needs_download_url': {}".format(needs_download_url))
    self.logger.info("function argument 'needs_date': {}".format(needs_date))
    self.logger.info("function argument 'needs_print_settings': {}".format(needs_print_settings))
    self.logger.info("function argument 'needs_sort': {}".format(needs_sort))
    self.logger.info("generated_file_id: {}".format(generated_file_id))
    self.logger.info("generated_filename: {}".format(generated_filename))
    self.logger.info("local_workspace_path: {}".format(local_workspace_path))
    ...
    objs = self.__s3_repo.list_objs(EXCEL_TEMPLATES)
    for obj in objs:
    ...
    excel.generate(
      ...
      needs_print_settings=needs_print_settings,
  ...
  def gen_download_url(self, generated_file_id, project_id=None):
    ...
    return { 
        "download_url": url,
        "name": b64decode(tags["name"]).decode("utf-8")
    }
