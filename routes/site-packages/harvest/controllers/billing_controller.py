...
class BillingController(DynamoDbDriver):
  ...
  def list_history(self, user_id=None):
    ...
    sorted_items = sorted(
        [ item["history"] for item in ret ],
        key=lambda item: datetime.strptime(item["invoice_date"], '%Y/%m/%d'), 
        reverse=False
    )
    self.logger.info("billing history data: %s" % str(sorted_items))
    return sorted_items

  def create(self, context, period, invoice_date, invoice_number, user_id=None, raw_price=0, tax=1.10, users_count=0):
    if not user_id:
      user_id = self.user_id
  ...
  def update(self, history, raw_price, users_count, tax=1.10, user_id=None):
    if not user_id:
      user_id = self.user_id
    ...
    return super().update_item(
        ...
        sort_key = history,
        update_exp = "set history.#price = :price, history.#raw_price = :raw_price, history.#users_count = :users_count",
        exp_attr_names = {
            '#price': "price",
            '#raw_price': "raw_price",
            '#users_count': "users_count"
        ...
        exp_attr_values={
            ':price': floor(raw_price * Decimal(tax)),
            ':raw_price': Decimal(raw_price),
            ':users_count': users_count
        },
