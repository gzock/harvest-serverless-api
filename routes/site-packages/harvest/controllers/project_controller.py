...
class ProjectController(DynamoDbDriver):
  ...
  def set_user_id(self, user_id):
    ...
    self.__projects_id = self.list_projects_id()
  ...
  def list_users(self):
    ...
    self.logger.info("specfied project users: " + users)
    for user in users:
      detail = cognito.show_user(user["user_id"])[0]["Attributes"]
      self.logger.info("fetched cognito user data: " + detail)

      detail = { item["Name"]: item["Value"] for item in detail }
      self.logger.info("proccesing cognito user data: " + detail)
  ...
  def show_join_code(self):
    if not self.user_id or not self.project_id:
      raise ValueError
    self.logger.info("specified project_id: " + self.project_id)
    join_code = urlsafe_b64encode(UUID(self.project_id).bytes).decode('ASCII').rstrip('=')
    self.logger.info("convert project_id to join_code: " + join_code)
    return { "join_code": join_code }

  def join_user(self):
    if not self.user_id or not self.project_id:
      raise ValueError
    return self.role.create("worker", project_id = self.project_id)
