...
from harvest.exception import TemplateNotFoundError, TemplateMaxCountExceededError, TemplateTypeInvalidError, DuplicateTemplateNameExistsError, NeedsPropertyNotExistsError, PageBreaksCountInvalidError, SettingPropertyCountInvalidError, PrintAreaNotExistsError
...
class TemplateController():
  ...
  def validate(self, template_path, property_title, property_hierarchy, property_date):
    ...
    for key, value in aggregate_cells.items():
      if len(value) != 1:
        raise SettingPropertyCountInvalidError(key, len(value))
    self.logger.info("setting propery count valid.")

    if len(excel.get_page_breaks(row_num_only=True)) not in [0, 1]:
