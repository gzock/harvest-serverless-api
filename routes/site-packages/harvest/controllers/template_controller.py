...
class TemplateController():
  ...
  def validate(self, template_path, property_title, property_hierarchy, property_date):
    ...
    print_area = rows_from_range(excel.get_print_area())
    print_area_cells = [ _cell for cells in print_area for _cell in cells ]

    for key, values in aggregate_cells.items():
      if len(values) != 1:
        raise SettingPropertyCountInvalidError(key, len(values))
      for cell in values:
        if cell.coordinate not in print_area_cells:
