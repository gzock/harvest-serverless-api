...
from harvest.exception import TemplateNotFoundError, TemplateMaxCountExceededError, TemplateTypeInvalidError, DuplicateTemplateNameExistsError, NeedsPropertyNotExistsError, PageBreaksCountInvalidError
...
class TemplateController():
  ...
  def create(
    ...
    if template_type == "user":
      ...
      list_templates = self.list_user_templates
    elif template_type == "project":
      upload_path = "%s%s/%s" % (PROJECTS_TEMPLATES_PATH, parent_id, new_id)
      list_templates = self.list_project_templates
    else:
      raise TemplateTypeInvalidError

    # TODO: プロジェクト側の最大数制限の処理も必要
    exists_objects = list_templates(upload_path)
    if len(exists_objects) >= 5:
      raise TemplateMaxCountExceededError
    if name in [ template["name"] for template in exists_objects ]:
