...
class ExcelDocGenerator():
  ...
  def copy_cells(self, src_row_min, src_row_max, dst_row_base, first_page_break_row, margin, count):
    ...
    for c in range(count):
      ...
      for merged_cell in copy(merged_cells):
        ...
        align = (merged_cell.min_row - first_page_break_row)
      ...
      dst_row_base = dst_row_base + index + margin
  ...
  def get_cell_width_px(self, cell):
    ...
    #print("###")
    #print(self.get_cell_width(cell))
    #print(points_to_pixels(self.get_cell_width(cell)))
    #print(round(self.get_cell_width(cell) * 8.08))
    #print("###")
    #return round(points_to_pixels(self.get_cell_width(cell)))
  ...
  def generate(self, title, items, save_filename, needs_include_hierarchy=False, needs_date=False, needs_print_settings=False, workspace="/tmp"):
    ...
    copy_ranges = {"src_row_min": 1, "src_row_max": 2, "dst_row_base": 2, "first_page_break_row": page_breaks[0], "margin": 1}
    self.logger.info("detect page_breaks: %s" % page_breaks)

    print_area = rows_from_range(self.ws.print_area[0])
    last_row = self.ws[list(print_area)[-1][0]].row
    ...
    if len(page_breaks) == 1:
      ...
      copy_ranges["first_page_break_row"] = 1
      copy_ranges["margin"] = last_row - page_breaks[0]
    elif len(page_breaks) == 2 and page_breaks[0] == 1:
      copy_ranges["src_row_min"] = 2
      copy_ranges["src_row_max"] = page_breaks[1]
      copy_ranges["dst_row_base"] = page_breaks[1] + 1
      copy_ranges["first_page_break_row"] = 2
      copy_ranges["margin"] = last_row - page_breaks[1]
